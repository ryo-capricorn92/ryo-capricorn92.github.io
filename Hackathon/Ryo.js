...
function Boss (name, type, desc1, desc2) {
	...
	this.health = 20;
	this.damage = 3;
	this.smallSprite = '<img class="bossSmall" src="imgs/sprites/trans.png"/>';
	this.bigSprite = '<img class="bossBig" src="imgs/sprites/trans.png"/>';
...
function fightMe(creature){
   ...
   //add random damage
   var personVariantDamage = getRandomArbitrary(1, 5);  
   var monsterVarintDamage = getRandomArbitrary(1, 3);  

   // set damage = to temp variables
   var personDmg = person['damage'];
  	personDmg += personVariantDamage;

   var creatureDmg = creature['damage'];
   	creatureDmg += 	monsterVarintDamage;
   ...
   while(lastManStanding&&creature.health > 0){
		console.log(person['name'] + ' attacks ' + creature['name'] +' for ' + personDmg + ' damage.');
		temp = creature['health'] - personDmg;
		console.log(creature['health'] + ' - ' + personDmg +  ' = ' + temp);
		creature['health'] = temp;

		console.log(creature['name'] + ' attacks ' + person['name'] +' for ' + creatureDmg + ' damage.');
		temp = person['health'] - creatureDmg;
		console.log(person['health'] + ' - ' + creatureDmg  + ' = ' + temp);
		person['health'] = temp;

	   if(creature['health'] <= 0 && person['health'] <= 0){
	   		$("#innerContainer").html('<img class="gameOver" src="http://i1138.photobucket.com/albums/n524/Bitmob/3%20Bitmob%20Random/Samir/GameOver.jpg" />');
		 ...
		 removeFallen.splice(removeFallen.indexOf(creature),removeFallen.indexOf(creature) + 1);
	    	...
	    	$("#outerContainer").html('<center><img class="gameOver" src="http://i1138.photobucket.com/albums/n524/Bitmob/3%20Bitmob%20Random/Samir/GameOver.jpg" /></center>');
	    	console.log('Our hero has fallen..');
	    	//<PlayAgain?> have dialogue to reset game?id="background"
	    }
	...
	if(all.stages[7].occupants[0] === undefined){
	$("#outerContainer").html('<center><img src="http://www.raywenderlich.com/wp-content/uploads/2013/02/006_YouWon.png" /></center>');
}	


 	$('#top').html('<center><table id="topTable" style="width: 90%"></table></center>');
	$('#topTable').append('<tr><td class="left">' + all.user.bigSprite + '</td><td class="right">' + creature.bigSprite + '</td></tr>');
	$('#topTable').append('<tr><td class="left"><div class="progress style="width: 70%;"><div id="userHealth" class="progress-bar progress-bar-success" style="width: ' + (all.user.health/30 * 100) + '%">' + all.user.health + '/30</div></div></td><td class="right"><div class="progress" style="width: 70%;"><div id="creatureHealth" class="progress-bar progress-bar-success" style="width: ' + (creature.health/creature.originalHealth * 100) + '%">' + creature.health + slash + creature.originalHealth + '</div></div></td></tr>');
}
...
function moveUser(location){
	...
	cantEscape = getRandomArbitrary(1, 100);
	...
	//if (all.numStages) {
		...
		for (var i = 0; i < numThings; i++) {
			...
			if (!random) {
				...
				var health2 = getRandomArbitrary(5, 8);
